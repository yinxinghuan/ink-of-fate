...
export function buildTattooImagePrompt(reading: FateReading): string {
  ...
  return [
    'HARD FULL-VISUAL-IDENTITY CAST MAP. REFERENCE IMAGE OVERRIDES ALL GENERIC WORDS.',
    'SUBJECT A MUST keep the exact complete visible identity and the reference must remain the full output frame, not artwork inside a new scene.',
    'Preserve the exact silhouette, form or species, proportions, material, head shape, face visibility, covering, mask, costume, colors, patterns, accessories, pose, camera angle, crop, background and lighting.',
    'Any face, skin, hair, hands, arms or legs not visible in the reference MUST remain hidden and MUST NOT be invented.',
    'Do not regenerate, humanize, idealize, restyle or replace SUBJECT A. A person, animal, cartoon, plant or object remains that exact same entity.',
    'CURRENT EDIT: make exactly one localized change.',
    `At ${reading.placement}, add a freshly inked tattoo depicting ${reading.tattoo_description}.`,
    `Tattoo treatment: ${styleVisual}. The mark follows the existing visible surface contour and material.`,
    'If the requested placement is not visibly supported, place the mark on the nearest already-visible surface; never reveal or add skin, limbs, hands or anatomy.',
    'Do not zoom out, reframe, relight, replace the background, add a tattoo parlor, add a person holding an object, or transfer identity traits to the tattoo or background.',
    'No new text, pseudo-text, logo, watermark, UI, border or frame. Preserve any pre-existing reference content unchanged.',
  ].join(' ');
}
