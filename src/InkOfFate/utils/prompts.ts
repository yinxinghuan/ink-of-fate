...
export function buildTattooImagePrompt(reading: FateReading): string {
  ...
  return [
    // ── FRAME LOCK ──
    `The reference image IS the full frame. You are EDITING that exact`,
    `frame in place. Do NOT zoom out, do NOT crop the reference into a`,
    `smaller region of a new larger photograph, do NOT compose a new`,
    `scene around it, do NOT show the reference as artwork sitting on`,
    `someone's skin or arm, do NOT use the reference's background as a`,
    `canvas surface. The boundaries of the reference are the boundaries`,
    `of the output. Even if the reference looks drawn / painted / printed`,
    `on paper / illustrated, treat it as a real photograph of that`,
    `subject and edit IN PLACE.`,
    ``,
    // ── IDENTITY LOCK ──
    `The subject in the reference — whatever it is (a person, animal,`,
    `cartoon character, plant, object, logo, anything) — must be KEPT`,
    `EXACTLY AS SHOWN. Same identity, same appearance, same proportions,`,
    `same pose, same colors, same surrounding environment, same camera`,
    `angle, same crop. The result must be obviously and unmistakably the`,
    `SAME SUBJECT from the reference, recognizable at first glance.`,
    `Do NOT regenerate the subject. Do NOT swap to a generic version.`,
    `Do NOT stylize, idealize, or "fix" anything. If it's a cat, it`,
    `stays the same cat in the same pose. If it's a mug, it stays the`,
    `same mug. If it's an illustrated avatar, it stays in that drawing`,
    `style.`,
    ``,
    // ── THE ONE EDIT ──
    `The ONLY change vs the reference: ON ${reading.placement}, ADD a`,
    `freshly inked tattoo: ${reading.tattoo_description}. The tattoo is`,
    `the ONLY new element. Everything else — subject, background,`,
    `lighting, color palette, crop — stays as in the reference.`,
    ``,
    // ── INK ──
    ...
    ``,
    // ── BIG REMINDER FOR NON-HUMAN SUBJECTS ──
    `If the subject is non-human (an animal, an object, a plant, a logo,`,
    `a cartoon), the tattoo still goes ON the subject like a real tattoo`,
    `or sticker applied to its surface. Do NOT add a hand or arm holding`,
    `the subject. Do NOT replace the subject with a person. Do NOT add`,
    `a person, a hand, or new skin to the frame. The subject is the`,
    `subject and the frame is the frame.`,
    ``,
    // ── LIGHTING ──
    `Lighting and atmosphere: preserve the reference photo's lighting`,
    `and background exactly. Do NOT relight, do NOT swap the background`,
    `to a tattoo parlor scene, do NOT add new objects to the room. If`,
    `the reference is on a plain paper background, the output also has`,
    `that plain paper background. Only the subject changes — and only at`,
    `${reading.placement}, by the addition of the new tattoo.`,
    ``,
